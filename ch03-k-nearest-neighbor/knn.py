...
def getClosest(trainDataMat, trainLabelMat, x, topK):
    ...
    #----------------优化点-------------------
    #由于我们只取topK小的元素索引值，所以其实不需要对整个列表进行排序，而 argsort 是对整个
...
def model_test(trainDataArr, trainLabelArr, testDataArr, testLabelArr, topK):
    ...
    trainLabelMat = np.array(trainLabelArr).T
    testDataMat = np.array(testDataArr)
    testLabelMat = np.array(testLabelArr).T

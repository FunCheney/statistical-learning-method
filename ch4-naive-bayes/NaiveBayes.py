...
def getAllProbability(trainDataArr, trainLabelArr):
    ...
    for i in range(classNum):
        ...
        Py[i] = ((np.sum(np.array(trainLabelArr) == i)) + 1) / (len(trainLabelArr) + 10)
    #转换为log对数形式
    # log书中没有写到，但是实际中需要考虑到，原因是这样：
    # 最后求后验概率估计的时候，形式是各项的相乘（“4.1 朴素贝叶斯法的学习” 式4.7），这里存在两个问题：
    # 1.某一项为0时，结果为0.这个问题通过分子和分母加上一个相应的数可以排除，前面已经做好了处理。
    # 2.如果特诊特别多（例如在这里，需要连乘的项目有784个特征加一个先验概率分布一共795项相乘，所有数都是0-1之间，结果一定是一个很小的接近0的数。）
    # 理论上可以通过结果的大小值判断， 但在程序运行中很可能会向下溢出无法比较，因为值太小了。所以人为把值进行log处理。
    # log在定义域内是一个递增函数，也就是说log（x）中，x越大，log也就越大，单调性和原数据保持一致。所以加上log对结果没有影响。此外连乘项通过
    # log以后，可以变成各项累加，简化了计算。在似然函数中通常会使用log的方式进行处理（至于此书中为什么没涉及，我也不知道）
    Py = np.log(Py)
